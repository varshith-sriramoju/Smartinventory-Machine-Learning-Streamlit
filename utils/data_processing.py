...
class DataProcessor:
    ...
    def process_uploaded_data(self, df, date_col, product_col, quantity_col, price_col=None, category_col=None, store_col=None,
        ...
        try:
            def _ensure_series(obj):
                # If a selection returns a DataFrame (duplicate labels), return the first column as a Series
            ...
            # Handle duplicate column names after renaming (avoid Series ops hitting a DataFrame)
            # Rebuild the DataFrame with unique columns, collapsing duplicates by taking first non-null across duplicates
            if processed_df.columns.duplicated().any():
                ...
                for name in unique_order:
                    block = processed_df.loc[:, processed_df.columns == name]
                    if isinstance(block, pd.DataFrame):
                        # Collapse duplicate columns horizontally: prefer leftmost non-null
                        collapsed = block.bfill(axis=1).iloc[:, 0]
                    else:
                        collapsed = block
                    combined[name] = collapsed

                processed_df = pd.DataFrame(combined)
            ...
            # Process product names (ensure Series, not DataFrame)
            prod_col = _ensure_series(processed_df['product_name'])
            processed_df['product_name'] = prod_col.astype(str).str.strip()
            ...
            if 'category' in processed_df.columns:
                cat_col = _ensure_series(processed_df['category'])
                processed_df['category'] = cat_col.astype(str).str.strip()
            ...
            if 'store_id' in processed_df.columns:
                store_col = _ensure_series(processed_df['store_id'])
                processed_df['store_id'] = store_col.astype(str).str.strip()
            else:
            ...
            # Include more context to help diagnose duplicate column issues
            try:
                cols = list(processed_df.columns) if 'processed_df' in locals() else []
                dtypes = processed_df.dtypes.astype(str).to_dict() if 'processed_df' in locals() else {}
                logger.error(f"Processing failed. Columns: {cols}\nDtypes: {dtypes}")
            except Exception:
                pass
    ...
    def clean_product_names(self, df):
        ...
        try:
            ...
            # Collapse duplicate 'product_name' columns if present and operate on a single Series
            block = df_clean.loc[:, df_clean.columns == 'product_name'] if 'product_name' in df_clean.columns else None
            if block is None or block.shape[1] == 0:
                return df_clean

            if isinstance(block, pd.DataFrame):
                prod = block.bfill(axis=1).iloc[:, 0]
            else:
                prod = block

            # Perform string normalizations on the Series
            prod = prod.astype(str).str.strip()
            prod = prod.str.title()
            prod = prod.str.replace(r'[^\w\s\-]', '', regex=True)
            prod = prod.str.replace(r'\s+', ' ', regex=True)

            # Drop duplicate 'product_name' columns and assign the cleaned Series back
            if df_clean.columns.duplicated().any():
                df_clean = df_clean.loc[:, ~df_clean.columns.duplicated(keep='first')]
            df_clean['product_name'] = prod
