...
class DatabaseManager:
    ...
    def _create_tables(self):
        ...
        # MySQL DDL (split into individual statements)
        ddl_statements = [
            """
        ...
        index_statements = [
            # MySQL doesn't support IF NOT EXISTS for CREATE INDEX; catch and ignore duplicates
            "CREATE INDEX idx_sales_date ON sales_data(date)",
            "CREATE INDEX idx_sales_product ON sales_data(product_name)",
            "CREATE INDEX idx_forecasts_product_date ON forecasts(product_name, forecast_date)",
            "CREATE INDEX idx_inventory_product ON inventory_recommendations(product_name)",
            "CREATE INDEX idx_performance_product ON model_performance(product_name)",
        ]
            ...
            with self.engine.connect() as conn:
                ...
                for stmt in index_statements:
                    try:
                        ...
                        pass
                conn.commit()
            logger.info("Database tables created successfully (MySQL)")
        except Exception as e:
    ...
    def clear_all_data(self):
        ...
        try:
            if self.engine:
                with self.engine.connect() as conn:
                    # MySQL: TRUNCATE tables individually
                    for table in [
                        'sales_data',
                        'forecasts',
                    ...
                    conn.commit()
                logger.info("All database tables cleared (MySQL)")
            else:
                # Clear session state
                st.session_state.sales_data = pd.DataFrame()
                st.session_state.forecasts = pd.DataFrame()
