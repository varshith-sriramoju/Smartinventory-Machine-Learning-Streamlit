...
class ForecastingEngine:
    ...
    def generate_forecast(self, product_data, forecast_days, model_type='Random Forest', confidence_level=95):
        ...
        try:
            ...
            if future_features.size == 0:
                logger.warning("Could not create future features")
                return None
            ...
            std_error = np.std(y_test - y_pred_test) if len(y_test) > 0 else np.std(y_train - model.predict(X_train_scaled))
            z_score = 1.96 if confidence_level == 95 else 2.58  # 95% or 99% confidence
            forecast_df['confidence_lower'] = np.maximum(forecast_df['predicted_quantity'] - z_score * std_error, 0)
            forecast_df['confidence_upper'] = forecast_df['predicted_quantity'] + z_score * std_error
    ...
    def _create_future_features(self, data, forecast_days):
        ...
        try:
            ...
            for i, future_date in enumerate(future_dates):
                ...
                if i > 0:
                    for j in range(i):
                        # For simplicity, use the mean of recent data as prediction placeholder
                        current_data.append(np.mean(current_data[-7:]) if len(current_data) >= 7 else current_data[-1])
